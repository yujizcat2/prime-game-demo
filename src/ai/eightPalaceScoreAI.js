...
export function summarizeScoreResults(results){
  ...
  const daySummaries = Array.from({length: maximumDay}, (_, index) => index + 1).map(day => {
    ...
    return {
      day,
      targetScore: getDayTargetScore(day),
  ...
  return {
    ...
    comboBonusScoreRatio: results.reduce((sum, result) => sum + (result.comboBonusTotal ?? 0), 0)
      / Math.max(1, results.reduce((sum, result) => sum + (result.finalScore ?? 0), 0)),
