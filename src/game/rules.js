...
export function canCombine(a,b,numbers=[]){const aType=a?.foodType==="meat"?FOOD_TYPES.LAND:a?.foodType,bType=b?.foodType==="meat"?FOOD_TYPES.LAND:b?.foodType;const hasDrink=aType===FOOD_TYPES.DRINK||bType===FOOD_TYPES.DRINK,isBurst=hasDrink&&(a?.value??0)+(b?.value??0)>202;return Boolean(!(aType===FOOD_TYPES.DRINK&&bType===FOOD_TYPES.DRINK)&&(numbers.length<9||isBurst)&&canCombineRelation(a,b,numbers)&&(isNormalFoodType(aType)||aType===FOOD_TYPES.DRINK)&&(isNormalFoodType(bType)||bType===FOOD_TYPES.DRINK));}
