...
export async function runFixedEightPalaceAttempts({attempts=10,depth=EIGHT_PALACE_SOLVER_DEFAULTS.depth,beamWidth=EIGHT_PALACE_SOLVER_DEFAULTS.beamWidth,maxActions=EIGHT_PALACE_SOLVER_DEFAULTS.maxActions,onProgress=null,fixedOpening=null}={}){
  const opening=deepFreeze(deepClone(fixedOpening??createEightPalaceInitialValues()));
  const results=[];
  for(let attemptIndex=1;attemptIndex<=attempts;attemptIndex++){
    const result=await runEightPalaceGame({depth,beamWidth,maxActions,initialOpening:deepClone(opening),randomizeSearch:true});
    result.attemptIndex=attemptIndex;result.routeSignature=routeSignature(result);results.push(result);
    onProgress?.({completed:attemptIndex,total:attempts,currentGame:result,currentActions:result.actions,currentSteps:result.steps,currentBoardCount:result.finalBoardCount,currentKeyCount:result.finalKeyCount,currentSuccess:result.success});
    await new Promise(resolve=>setTimeout(resolve,0));
  }
