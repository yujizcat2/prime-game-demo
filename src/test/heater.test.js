...
const hundred = {...createGameState(opening([100])), money: 10};
...
const historyState = createGameState(opening([17]));
...
const reducible = {...createGameState(opening([17, 6])), money: 10};
...
const deadButFunded = resolveGameOver({...createGameState(opening([17])), money: 10});
assert.equal(deadButFunded.gameOver, false);
const deadAndPoor = resolveGameOver({...createGameState(opening([17])), money: 0});
assert.equal(deadAndPoor.gameOver, true);
const revived = resolveGameOver(applyHeater({
  ...createGameState(opening()), money: 10, gameOver: true, gameOverReason: "no_legal_actions"
}, 0));
assert.equal(revived.gameOver, false);

assert.equal(createGameState(opening()).heaterUseCount, 0);
assert.equal(getHeaterCost(createGameState(opening())), 10);
