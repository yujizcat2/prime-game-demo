...
assert.deepEqual(reducePreview.results.map(piece=>getFoodName(piece.value,piece.foodType)),[getFoodName(11,T.LAND),getFoodName(2,T.LAND)]);assert.ok(reducePreview.results.every(piece=>piece.foodType!==T.DRINK));
const reverseReducePreview=createReduceOutcome(drinkReduceState,7,1);assert.deepEqual(reverseReducePreview.results.map(piece=>[piece.value,piece.foodType]),[[2,T.LAND],[11,T.LAND]]);assert.deepEqual(reverseReducePreview.results.map(piece=>getFoodName(piece.value,piece.foodType)),[getFoodName(2,T.LAND),getFoodName(11,T.LAND)]);
