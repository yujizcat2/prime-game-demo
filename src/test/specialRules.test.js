...
const drinkReduceState=drinkState(110,20,T.LAND),reducePreview=createReduceOutcome(drinkReduceState,1,7),drinkReduced=applyAction(drinkReduceState,{type:"reduce",indexes:[1,7]});assert.deepEqual(reducePreview.results.map(item=>[item.value,item.foodType]),[[11,T.LAND],[2,T.SEASONING]]);assert.equal(drinkReduced.board[1].value,11);assert.equal(drinkReduced.board[1].foodType,T.LAND);assert.equal(drinkReduced.board[7].value,2);assert.equal(drinkReduced.board[7].foodType,T.SEASONING);
assert.deepEqual(reducePreview.results.map(piece=>getFoodName(piece.value,piece.foodType)),[getFoodName(11,T.LAND),getFoodName(2,T.SEASONING)]);assert.ok(reducePreview.results.every(piece=>piece.foodType!==T.DRINK));
const reverseReducePreview=createReduceOutcome(drinkReduceState,7,1);assert.deepEqual(reverseReducePreview.results.map(piece=>[piece.value,piece.foodType]),[[2,T.SEASONING],[11,T.LAND]]);assert.deepEqual(reverseReducePreview.results.map(piece=>getFoodName(piece.value,piece.foodType)),[getFoodName(2,T.SEASONING),getFoodName(11,T.LAND)]);
...
const vegetableReduced=applyAction(drinkState(110,20,T.VEGETABLE),{type:"reduce",indexes:[7,1]});assert.equal(vegetableReduced.board[1].foodType,T.VEGETABLE);assert.equal(vegetableReduced.board[7].foodType,T.SEASONING);
const drinkCollectPreview=createReduceOutcome(drinkState(110,10,T.LAND),1,7);assert.deepEqual(drinkCollectPreview.results.map(piece=>[piece.value,piece.foodType]),[[11,T.LAND],[1,T.LAND]]);const drinkCollect=applyAction(drinkState(110,10,T.LAND),{type:"reduce",indexes:[1,7]});assert.equal(drinkCollect.board[1].value,11);assert.equal(drinkCollect.board[7],null);assert.ok(drinkCollect.collectionCards.some(card=>card.value===10));
const centerDrinkReduce=applyAction(drinkState(110,20,T.VEGETABLE,1,4),{type:"reduce",indexes:[1,4]});assert.equal(centerDrinkReduce.board[4].value,2);assert.equal(centerDrinkReduce.board[4].foodType,T.VEGETABLE);
const nativeDrinkReduce=applyAction(drinkState(110,20,T.SEASONING),{type:"reduce",indexes:[1,7]});assert.equal(nativeDrinkReduce.board[7].value,2);assert.equal(nativeDrinkReduce.board[7].foodType,T.SEASONING);
...
const simDrinkReduce=createSimulationState([110,20,7],"eightPalace");simDrinkReduce.board[0].foodType=T.DRINK;simDrinkReduce.board[1].foodType=T.LAND;assert.equal(applySimulationAction(simDrinkReduce,{type:"reduce",indexes:[0,1]}),true);assert.equal(simDrinkReduce.board[1].value,2);assert.equal(simDrinkReduce.board[1].foodType,T.AQUATIC);
