...
const equalSameBase=eightStateWith([{value:7,foodType:T.VEGETABLE},{value:7,foodType:T.VEGETABLE}]),equalSamePreview=createReduceOutcome(equalSameBase,0,1),equalSame=applyAction(equalSameBase,{type:"reduce",indexes:[0,1]});assert.equal(equalSamePreview.kind,"equalRetype");assert.equal(equalSame.board[0].id,equalSameBase.board[0].id);assert.equal(equalSame.board[0].value,7);assert.equal(equalSame.board[0].foodType,T.VEGETABLE);assert.equal(equalSame.board[1],null);assert.equal(equalSame.collectionCards.length,0);assert.equal(equalSame.score,equalSameBase.score);assert.equal(equalSame.steps,equalSameBase.steps+1);
const equalMixedBase=eightStateWith([{value:7,foodType:T.VEGETABLE},{value:7,foodType:T.AQUATIC}]),equalMixedFirstId=equalMixedBase.board[0].id,equalMixed=applyAction(equalMixedBase,{type:"reduce",indexes:[0,1]});assert.equal(equalMixed.board[0].id,equalMixedFirstId);assert.equal(equalMixed.board[0].value,7);assert.equal(equalMixed.board[0].foodType,T.AQUATIC);assert.equal(equalMixed.board[1],null);assert.equal(equalMixed.collectionCards.length,0);assert.equal(equalMixed.score,equalMixedBase.score);
const equalReverseBase=eightStateWith([{value:7,foodType:T.AQUATIC},{value:7,foodType:T.VEGETABLE}]),equalReverse=applyAction(equalReverseBase,{type:"reduce",indexes:[0,1]});assert.equal(equalReverse.board[0].id,equalReverseBase.board[0].id);assert.equal(equalReverse.board[0].value,7);assert.equal(equalReverse.board[0].foodType,T.VEGETABLE);assert.equal(equalReverse.board[1],null);assert.equal(equalReverse.collectionCards.length,0);
...
const wrapState=drinkState(190,20),wrapPreview=createCombineOutcome(wrapState,1,7),wrapped=applyAction(wrapState,{type:"combine",indexes:[1,7]});assert.equal(wrapPreview.kind,"wrap");assert.equal(wrapPreview.piece.value,10);assert.equal(wrapPreview.piece.foodType,T.LAND);assert.equal(wrapped.board[1].value,10);assert.equal(wrapped.board[1].foodType,T.LAND);assert.equal(wrapped.board[7].value,20);assert.equal(wrapped.board.filter(Boolean).length,2);assert.equal(wrapped.collectionCards.length,0);assert.equal(wrapped.score,wrapState.score);
...
const wrappedReverse=applyAction(wrapState,{type:"combine",indexes:[7,1]});assert.equal(wrappedReverse.board[1].value,10);assert.equal(wrappedReverse.board[1].foodType,T.LAND);assert.equal(wrappedReverse.board[7].value,20);
const vegetableWrap=applyAction(drinkState(202,37,T.VEGETABLE),{type:"combine",indexes:[1,7]});assert.equal(vegetableWrap.board[1].value,39);assert.equal(vegetableWrap.board[1].foodType,T.VEGETABLE);
const fruitWrap=applyAction(drinkState(180,30,T.FRUIT),{type:"combine",indexes:[1,7]});assert.equal(fruitWrap.board[1].value,10);assert.equal(fruitWrap.board[1].foodType,T.FRUIT);
...
const fullWrapBoard=drinkState(190,20);for(let index=0;index<9;index++)if(!fullWrapBoard.board[index])fullWrapBoard.board[index]={id:900+index,value:43+index,foodType:T.FRUIT};assert.ok(getLegalActions(fullWrapBoard).some(action=>action.type.startsWith("combine")&&action.indexes.includes(1)&&action.indexes.includes(7)));const fullWrapped=applyAction(fullWrapBoard,{type:"combine",indexes:[1,7]});assert.equal(fullWrapped.board[1].value,10);assert.equal(fullWrapped.board[7].value,20);assert.equal(fullWrapped.board.filter(Boolean).length,9);
