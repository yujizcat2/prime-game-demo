...
const equalDrinkPair=createSimulationState([120,120,7],"eightPalace");equalDrinkPair.board[0].foodType=T.DRINK;equalDrinkPair.board[1].foodType=T.DRINK;assert.equal(getSimulationLegalActions(equalDrinkPair).some(action=>action.indexes?.includes(0)&&action.indexes?.includes(1)),false);assert.equal(applySimulationAction(equalDrinkPair,{type:"reduce",indexes:[0,1]}),false);
