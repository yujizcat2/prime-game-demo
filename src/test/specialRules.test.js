...
const repeatCollectState={...drinkCollect,gameOver:false,board:drinkCollectState.board.map(piece=>piece?{...piece}:null)};const repeatedScore=repeatCollectState.score,repeatedMoney=repeatCollectState.money,repeatedCards=repeatCollectState.collectionCards.length;const repeatedCollect=applyAction(repeatCollectState,{type:"reduce",indexes:[3,1]});assert.equal(repeatedCollect.board[3].foodType,T.GRAIN_BEAN);assert.equal(repeatedCollect.board[1],null);assert.equal(repeatedCollect.score,repeatedScore);assert.equal(repeatedCollect.money,repeatedMoney);assert.equal(repeatedCollect.collectionCards.length,repeatedCards);
