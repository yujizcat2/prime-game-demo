...
const rewardToastSource = readFileSync("src/components/CollectionSaleToast.jsx", "utf8");
assert.match(rewardToastSource, /售出料理/);
assert.match(rewardToastSource, /基础/);
assert.match(rewardToastSource, /额外/);
assert.doesNotMatch(rewardToastSource, /确认|createPortal|saleBreakdown/);
const rewardToastCssSource = readFileSync("src/components/CollectionSaleToast.css", "utf8");
assert.match(rewardToastCssSource, /pointer-events:\s*none/);
assert.match(rewardToastCssSource, /1\.2s/);
const appSource = readFileSync("src/App.jsx", "utf8");
assert.match(appSource, /rewards\.filter\(reward => reward\.isNewCollection\)/);
assert.match(appSource, /setTimeout\([\s\S]*1200/);
