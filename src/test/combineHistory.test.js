...
assert.notEqual(createCombinePairKey({...a,id:1},{...b,id:2}),createCombinePairKey({...a,id:3},{...b,id:4}),"different card instances do not share a pair lock");
...
assert.equal(hasCombinePair(state.combineHistoryKeys,state.board[0],state.board[1]),false,"new card instances do not inherit a value/type pair lock");
assert.equal(canCombineCells({...state,gameOver:false},0,1),true,"formal predicate allows fresh cards with previously used identities");
assert.equal(getActionStatus(state.board.filter(Boolean),[901,902],state.combineHistoryKeys).combine.allowed,true,"UI status uses the same instance-pair rule");
...
generatedTypeLocked.mergeHistoryByIdentity={"card:561":[{id:900,value:2,foodType:predicted,name:"旧料理",role:"result"}]};
...
reducedIdentity.mergeHistoryByIdentity={
  "card:1201":[{id:999,value:99,foodType:T.FRUIT,name:"本卡直接历史",role:"partner"}],
...
assert.deepEqual(historyOf(reducedIdentity,reducedIdentity.board[0]).map(item=>item.name),["本卡直接历史"],"reduction preserves only the card instance's direct history");
assert.equal(historyOf(reducedIdentity,reducedIdentity.board[0]).some(item=>item.name==="旧身份共享历史"),false,"reduction does not adopt another card's value/type history");

const sharedIdentity=placed([piece(4,T.GRAIN_BEAN,{id:4101}),piece(4,T.GRAIN_BEAN,{id:4102}),piece(5,T.FRUIT,{id:5101})]);
sharedIdentity.mergeHistoryByIdentity={"card:4101":[{id:5101,value:5,foodType:T.FRUIT,name:"直接搭档",role:"partner"}]};
assert.notDeepEqual(historyOf(sharedIdentity,sharedIdentity.board[0]),historyOf(sharedIdentity,sharedIdentity.board[1]),"same value and food type on different cards keep separate direct histories");
assert.equal(canCombineCells(sharedIdentity,1,2),true,"a sibling card does not inherit another instance's history");
...
const existingKey=`card:${existingOutcome.piece.id}`;
...
const separatedTypes={...sharedIdentity,mergeHistoryByIdentity:{"card:4101":[{id:800,value:8,foodType:T.LAND,name:"谷物历史",role:"partner"}]}};
...
generations.mergeHistoryByIdentity[`card:${child.id}`].push({id:9999,value:7,foodType:T.AQUATIC,name:"同数字旧记录",role:"parent"});
...
assert.equal(getSimulationLegalActions(sim).some(action=>action.type.startsWith("combine")&&action.indexes.includes(0)&&action.indexes.includes(1)),true,"simulation allows fresh cards with the same values/types");
assert.equal(applySimulationAction(sim,{type:"combine",indexes:[1,0]}),true);
