...
export function getFoodDisplayName(item,fallbackFoodType=null){if(item?.value==null)return null;if(item.value===1)return "水";return item.displayName??item.name??getFoodName(item.value,item.foodType??item.type??fallbackFoodType);}
